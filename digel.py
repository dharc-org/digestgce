...
def get_entities(what):
  ...
  q = Q_PEOPLE if what == 'people' else Q_JOURNALS if what == 'journals' else Q_PEOPLE
...
def create_graphs(articles_df,authors_uri,journals_uri,filename,api_key, match_people=None, match_journals=None):
        ...
        if 'book chapter' in row["publication type"].lower() or ('incollection' in row["publication type"].lower() and row["journal"] == 'nan') or 'misc' in row["publication type"].lower():
          ...
          if "booktitle" in row:
            if row["booktitle"] != 'nan' and len(row["booktitle"]) > 1:
              book_uri = books_uri[row["booktitle"].strip()]
        ...
        if 'thesis' in row["publication type"].lower() or 'doctoral theses' in row["publication type"].lower() \
          ...
          if "publisher" in row:
            if row["publisher"] != 'nan' and len(row["publisher"]) > 1:
              ...
              publisher_uri = publishers_uri[publisher.strip()]
  ...
  g.serialize(format='trig',destination=filename+'.trig',  encoding='utf-8' )

  # download
  files.download(filename+'.trig')
